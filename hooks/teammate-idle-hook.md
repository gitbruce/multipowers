...
This hook enables event-driven agent scheduling for Multipowers workflows.

## Purpose

When a teammate agent finishes its current work and enters an idle state, this hook:

1. Checks for an active multipowers workflow phase
...
- `~/.multipowers/session.json` - Workflow session state with agent queue
- `.claude-plugin/bin/mp` - Main orchestration (populates agent queue)
