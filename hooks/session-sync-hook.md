...
event: PreToolUse
tools: ["Bash"]
description: Syncs Claude session context before mp runtime execution
---

# Session Sync PreToolUse Hook

This hook ensures Claude Code's session ID propagates to mp runtime invocations for cross-session tracking.

## Purpose

When Claude Code invokes mp runtime via the Bash tool, this hook:

1. Detects mp runtime invocations
...
- Tool is Bash
- Command contains `mp runtime`

## Session ID Propagation (Claude Code v2.1.10)

The `${CLAUDE_SESSION_ID}` environment variable is available in Claude Code. This hook ensures it's properly passed through to mp runtime:

```bash
# mp runtime will detect this and use it for session tracking
...
1. **Cross-Session Correlation**: Track multiple mp runtime runs within the same Claude Code session
