---
command: plan
description: "Intelligent plan builder - creates strategic execution plans (doesn't execute). Use /mp:embrace to execute plans."
aliases:
  - build-plan
  - intent
---
...
**Creates strategic execution plans based on user intent. Saves plans for review and optional execution with /mp:embrace.**

## Key Behavior

- **Creates plans** - Captures intent, analyzes requirements, generates weighted execution strategy
- **Saves to files** - Stores plan (`.multipowers/tracks/session-plan.md`) and intent contract (`.multipowers/tracks/session-intent.md`)
- **Doesn't execute** - Plans are saved for review; execution requires user confirmation
- **Optional execution** - Can invoke `/mp:embrace` immediately or user can execute later

## 🤖 INSTRUCTIONS FOR CLAUDE

When the user invokes this command (e.g., `/mp:plan <arguments>`):
...
1. Create `.multipowers/tracks/session-intent.md` with:
...
**When to use /mp:plan (octopus workflows):**
...
→ /mp:discover (extended depth)

DEFINE ████ 15%
Lock the scope — Confirm boundaries and approach
→ /mp:define (light touch)
...
1. **Save plan to `.multipowers/tracks/session-plan.md`:**

```markdown
# Session Plan

**Created:** [timestamp]
**Intent Contract:** See .multipowers/tracks/session-intent.md
...
\`\`\`bash
/mp:embrace "[user's goal]"
\`\`\`

Or execute phases individually:
- `/mp:discover` (if Discover > 20%)
- `/mp:define` (if Define > 20%)
- `/mp:develop` (if Develop > 20%)
- `/mp:deliver` (if Deliver > 20%)
...
## Next Steps
1. Review this plan
2. Adjust if needed (re-run /mp:plan)
3. Execute with /mp:embrace when ready
```
...
```
✅ Plan saved to .multipowers/tracks/session-plan.md

To execute this plan, run:
  /mp:embrace "[user's goal]"

Or adjust the plan:
  /mp:plan  (re-run to modify)
```
  ...
  questions: [
    {
      ...
      multiSelect: false,
      options: [
        {label: "Review and execute later", description: "Plan saved, I'll run /mp:embrace when ready (Recommended)"},
        {label: "Adjust plan weights", description: "Change phase emphasis before saving"},
        {label: "Execute now", description: "Run /mp:embrace immediately with this plan"},
...
- Save plan and exit
- User can review `.multipowers/tracks/session-plan.md` at their leisure
- User runs `/mp:embrace` when ready
...
**If "Execute now":**
- Invoke `/mp:embrace` skill with the user's goal
...
### Step 7: Integration with /mp:embrace (Optional Execution)

**If user chose "Execute now" in Step 6:**

The plan command should invoke the `/mp:embrace` skill, which handles:
...
**Important:** The plan command itself does NOT execute workflows. It delegates to `/mp:embrace` for execution.

### Step 8: Plan Command Completes

**The plan command exits after:**
- Creating and saving the plan (`.multipowers/tracks/session-plan.md`)
- Creating the intent contract (`.multipowers/tracks/session-intent.md`)
- Optionally invoking `/mp:embrace` if user requested immediate execution

**The plan command does NOT:**
- Execute workflows directly (delegates to `/mp:embrace`)
- Validate results (that's `/mp:embrace`'s responsibility)
- Implement anything (that's what workflows do)

**Clear separation of concerns:**
- `/mp:plan` → Creates strategic plans
- `/mp:embrace` → Executes plans through 4-phase workflow
...
→ Invokes /mp:embrace immediately
...
```
User: /mp:plan "Should we use Redis or PostgreSQL?"

[After 5 questions show decision need]

Claude presents plan:
→ Recommends /mp:debate for this type of decision
...
→ Invokes /mp:debate instead of /mp:embrace
```
...
2. Creates `.multipowers/tracks/session-intent.md`
...
- **Execute when ready** - run `/mp:embrace` at your own pace
...
**Workflow Separation:**
- `/mp:plan` → Strategic planning (creates plans, doesn't execute)
- `/mp:embrace` → Execution (runs the full 4-phase workflow)
- Individual phases → Execute specific phases independently

---

**Ready to use!** Users can invoke with `/mp:plan` to create customized execution plans, then execute with `/mp:embrace` when ready.
