...
When the user invokes this command (e.g., `/mp:embrace <arguments>`):
...
- Suggest running `/mp:setup` to configure missing providers
...
```bash
/mp:embrace        # Full workflow with natural language
```
...
Set in `/mp:setup` or use orchestrate.sh flags:
```bash
...
- `/mp:discover` - Run just the research phase
- `/mp:define` - Run just the definition phase
- `/mp:develop` - Run just the development phase
- `/mp:deliver` - Run just the delivery/review phase
