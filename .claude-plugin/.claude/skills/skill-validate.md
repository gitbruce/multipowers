...
# Validate directory
/mp:validate src/components/

# Validate with focus area
/mp:validate api/ --focus security

# Validate against reference
/mp:validate src/ --reference extraction-results/
```
...
When the user invokes `/mp:validate <target>`, you MUST follow these steps sequentially. Each step is BLOCKING - you CANNOT skip or simulate any step.
...
2. Export to PPTX/PDF (via /mp:docs)
...
```bash
# Build feature
/mp:develop user authentication

# Validate before commit
/mp:validate src/auth/ --focus security
```

### With /mp:debate
```bash
# Use debate for architectural decisions
/mp:debate "Should we use JWT or sessions?"

# Validate chosen implementation
/mp:validate src/auth/ --focus best-practices
```
