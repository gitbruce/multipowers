...
You can return to current state with:
  /mp:rollback octo-checkpoint-pre-rollback-$TIMESTAMP
```
...
| Command | Action |
|---------|--------|
| `/mp:rollback` | List available checkpoints |
| `/mp:rollback list` | List available checkpoints |
| `/mp:rollback <tag>` | Rollback to specific checkpoint |
