...
1. **Built-in Claude Code commands** - Commands starting with `/` that are part of Claude Code itself:
   ...
   - Any other `/` command that isn't `/parallel-agents` or `/mp:*`
...
Force multi-provider execution using the `/mp:multi` command:

```
/mp:multi "Explain how Redis works"
/mp:multi "What is OAuth?"
/mp:multi "Review this simple function"
/mp:multi "Should we use TypeScript?"
```
...
> - 'Review this code for security issues'
>
> Need guided setup? Run `/mp:setup`"
...
> ```
>
> After installing one, run `/mp:setup` to verify everything works."
