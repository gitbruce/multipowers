...
If user provided a description inline with the command (e.g., `/mp:spec user authentication system`), use that as the project description but STILL ask remaining questions (actors, constraints, complexity).
...
- Prior discover phases (if user ran `/mp:discover` first)
...
- Use /mp:develop to implement from this spec
- Use /mp:embrace for full lifecycle from spec
```
...
- **Step 2 (Providers)**: If both unavailable, suggest `/mp:setup` and STOP
