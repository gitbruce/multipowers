...
**Do NOT use for:**
- Starting new projects (use /mp:embrace)
- Checking current status only (use /mp:status)
...
Run `/mp:embrace [your project description]` to start a new project.

**Example:**
```
/mp:embrace build a REST API with user authentication
```
...
| `blocked` | Review blockers | "Review blockers first: `/mp:issues`" |
| `complete` | Ready for next phase | "Phase complete. Ready for `/mp:ship`" |
| `paused` | Resume project | "Project paused. Resume with `/mp:embrace`" |
| `ready` | Begin workflow | "Ready to begin. Run `/mp:embrace` to start." |
| `planning` | Continue planning | "Continue planning. Use `/mp:define` to refine." |
| `building` | Continue building | "Continue implementation. Use `/mp:develop`." |
...
Review blockers first: `/mp:issues`
...
**Good:**
```
Status: blocked → "Review blockers first: /mp:issues"
```
...
```
/mp:resume → Full restoration with context
/mp:status → Quick dashboard without restoration
```

### With /mp:embrace

```
No .octo/ exists → /mp:resume suggests /mp:embrace
.octo/ exists but paused → /mp:resume suggests resuming with /mp:embrace
```
...
User runs /mp:resume
→ Context restored
→ User continues with /mp:develop (or appropriate phase skill)
```
