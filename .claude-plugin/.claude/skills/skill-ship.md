...
**Do NOT use for:**
- Code review (use /mp:review or flow-deliver)
- Implementation (use /mp:develop)
- Research (use /mp:research)
...
Run `/mp:embrace [description]` to start a new project.
```
...
1. Complete remaining phases:
   - [ ] Phase 1: Discover - Use `/mp:discover`
   - [ ] Phase 2: Define - Use `/mp:define`
   - [ ] Phase 3: Develop - Use `/mp:develop`
   - [ ] Phase 4: Deliver - Use `/mp:deliver`
...
Resolve issues and run `/mp:ship` again.
```
...
Restore with: `/mp:rollback {CHECKPOINT_TAG}`

---

**To start a new project:** `/mp:embrace`
...
| Error | Resolution |
|-------|------------|
| No .octo/ directory | Suggest `/mp:embrace` |
...
```
User runs /mp:deliver to validate
→ Deliver phase complete
→ User runs /mp:ship to finalize
```
...
User ships project
→ Checkpoint created
→ Later: /mp:rollback octo-checkpoint-shipped-*
```
...
→ Suggests /mp:embrace for new project
```
