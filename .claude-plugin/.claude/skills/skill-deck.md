...
trigger: |
  ...
  DO NOT activate for:
  - Converting existing markdown to PPTX (use /mp:docs instead)
...
- For deeper multi-AI research, suggest: "For comprehensive multi-provider research, run `/mp:discover [topic]` first, then `/mp:deck` to build slides from the results"
...
- `/mp:discover [topic]` — Research first, then build deck from findings
- `/mp:docs` — For converting existing markdown to other formats
- `/mp:prd` — Generate a PRD, then create a presentation summarizing it
- `/mp:brainstorm` — Brainstorm ideas, then structure into a deck
...
→ Suggest: "/mp:discover autonomous vehicles market" first
→ Then: "/mp:deck" using the research synthesis
```

---

*Slide deck skill for multipowers v8.12.0+*
