...
This skill provides **automatic context detection** to determine whether the user is working in a **Development context** (code-focused) or **Knowledge context** (research/strategy-focused). This replaces the manual `/mp:km` toggle with intelligent auto-detection.
...
If user has explicitly set mode via `/mp:km on` or `/mp:km off`, respect that setting.
...
> "I detected this as a [dev/knowledge] task. If that's wrong, you can use `/mp:km` to override."
...
3. With `/mp:km on` set, ask "octo research API patterns" → Should use **Knowledge Context** (explicit override)
