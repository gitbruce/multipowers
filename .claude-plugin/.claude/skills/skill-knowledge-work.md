...
3. **Explicit Override** (if set via `/mp:km`):
...
To override: /mp:km on (force Knowledge) or /mp:km off (force Dev)
To return to auto: /mp:km auto
```
...
| Command | Description |
|---------|-------------|
| `/mp:km` | Show current status (auto, on, or off) |
| `/mp:km on` | Force Knowledge Context for all workflows |
| `/mp:km off` | Force Dev Context for all workflows |
| `/mp:km auto` | Return to auto-detection (default) |
...
- `/mp:discover` - Research workflow (auto-detects context)
- `/mp:develop` - Build workflow (auto-detects context)  
- `/mp:deliver` - Review workflow (auto-detects context)
- `/mp:docs` - Document export (works in both contexts)
