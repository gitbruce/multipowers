...
- 执行完成：`T01-T10`
- 拷贝保留脚本：`14`（均已落地到 `go` 分支）
- 迁移映射脚本：`131`（均已给出目标文件/目标符号，状态按实现进度维持 `TODO`）
- 基线一致性：`TOTAL=145` 与文档行数 `ROWS=145` 一致
- 策略一致性：`COPY_FROM_MAIN=14`，`MIGRATE_TO_GO=131`

未决阻塞：
- `internal/workflows` 的部分测试会在测试进程内再次调用 `go test ./...`，可能触发递归测试风暴；验证阶段采用 `-short` 作为当前最优安全验证策略，后续建议单独修复递归调用问题。
