...
**Architecture:** Keep upstream conflict surface minimal by placing new logic in `custom/*` and using thin wiring in `bin/mp`. Centralize spec-driven preflight and FAQ synthesis in reusable shell helpers; ensure all target-project writes stay under `/.multipowers/`.

**Tech Stack:** Bash (`bin/mp`, `custom/lib/*.sh`), Markdown templates (`custom/templates/*`), shell tests (`tests/unit/*.sh`, `tests/integration/*.sh`).
...
### Phase 2: Wire `/mp:init` to Generate Both Files in Target `/.multipowers/`
...
**Commit checkpoint:**
- [x] `git add bin/mp custom/config/setup.toml custom/lib/conductor-context.sh tests/unit/test-octo-init-render.sh tests/unit/test-claude-faq-init-render.sh && git commit -m "feat(init): generate CLAUDE and FAQ in target .multipowers"`
...
**Commit checkpoint:**
- [x] `git add bin/mp custom/docs/target-project/getting-started.md custom/docs/target-project/troubleshooting.md tests/unit/test-runtime-prerun-failfast.sh && git commit -m "feat(runtime): enforce fail-fast pre-run contract across execution paths"`
...
**Commit checkpoint:**
- [x] `git add bin/mp tests/unit/test-debate-routing.sh tests/integration/test-debate-integration.sh custom/docs/customizations/proxy-routing.md && git commit -m "fix(debate): enforce >=2 provider quorum with consistent fallback and proxy env"`
...
**Commit checkpoint:**
- [x] `git add custom/lib/faq-synthesizer.sh bin/mp tests/unit/test-faq-synthesizer.sh tests/integration/test-faq-auto-update.sh && git commit -m "feat(faq): auto-generate deduped multipowers FAQ by error type"`
...
- [x] P8.1 Run syntax checks
  - Run: `bash -n bin/mp && bash tests/smoke/test-syntax.sh`
